...
# Título do Dashboard
st.title("📈 Dashboard de Regressão Linear para Análise de Carros")

# Carregando os dados
@st.cache_data
def load_data():
    data = {
        'Marca': ['CURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA', 'ACURA'],
        'Modelo': ['ILX', 'ILX', 'ILX HYBRID', 'MDX 4WD', 'RDX AWD', 'RLX', 'TL', 'TL AWD', 'TL AWD', 'TSX'],
        'Tipo': ['COMPACT', 'COMPACT', 'COMPACT', 'SUV - SMALL', 'SUV - SMALL', 'MID-SIZE', 'MID-SIZE', 'MID-SIZE', 'MID-SIZE', 'COMPACT'],
        'Motor': [2, 2.4, 1.5, 3.5, 3.5, 3.5, 3.5, 3.7, 3.7, 2.4],
        'Portas': [4, 4, 4, 6, 6, 6, 6, 6, 6, 4],
        'Código1': ['AS5', 'M6', 'AV7', 'AS6', 'AS6', 'AS6', 'AS6', 'AS6', 'M6', 'AS5'],
        'Código2': ['Z']*10,
        'Feature1': [9.9, 11.2, 6, 12.7, 12.1, 11.9, 11.8, 12.8, 13.4, 10.6],
        'Feature2': [6.7, 7.7, 5.8, 9.1, 8.7, 7.7, 8.1, 9.0, 9.5, 7.5],
        'Feature3': [8.5, 9.6, 5.9, 11.1, 10.6, 10, 10.1, 11.1, 11.6, 9.2],
        'Feature4': [33, 29, 48, 25, 27, 28, 28, 25, 24, 31],
        'Preço': [196, 221, 136, 255, 244, 230, 232, 255, 267, 212]
    }
    ...
    numeric_columns = ['Motor', 'Portas', 'Feature1', 'Feature2', 'Feature3', 'Feature4', 'Preço']
...
# Verificar se a coluna 'Preço' está presente
target_column = 'Preço'
...
with st.sidebar.expander("🔍 Descrição das Features"):
    ...
    st.markdown("""
    - **Motor:** Tamanho do motor em litros.
    - **Portas:** Número de portas do veículo.
    - **Feature1:** Peso do veículo (em kg).
    - **Feature2:** Consumo de combustível (km/l).
    - **Feature3:** Potência do motor (HP).
    - **Feature4:** Velocidade máxima (km/h).
    - **Preço:** Preço do veículo (em milhares).
    """)
    ...
    if 'Heatmap' in viz_options:
        with st.expander("Heatmap de Features Específicas"):
            ...
            if heatmap_columns:
                ...
                ax.set_title("Heatmap de Features Específicas")
                st.pyplot(fig)
            else:
                st.warning("Selecione pelo menos uma coluna para visualizar o Heatmap.")
...
with tabs[2]:
    ...
    if len(selected_features) == 0:
        ...
        st.markdown("### Insira os valores das variáveis para obter uma previsão de preço:")
        ...
        if 'model' not in st.session_state:
            ...
            if st.button("Prever Preço"):
                ...
                try:
                    ...
                    st.success(f"O preço previsto é: **{prediction:.2f}**")
